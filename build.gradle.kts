...
tasks {
    ...
    val modsDeployDir = file("/home/studio/.local/share/PrismLauncher/instances/1.8.9/minecraft/mods")

    remapJar {
        inputFile.set(shadowJar.get().archiveFile)
        archiveClassifier.set("")
        // After producing the final remapped jar, copy it into the PrismLauncher mods folder.
        doLast {
            val out = archiveFile.get().asFile
            val deployName = "ZombiesPro.jar"
            copy {
                from(out)
                into(modsDeployDir)
                rename { deployName }
            }
            logger.lifecycle("Deployed $deployName -> $modsDeployDir")
        }
    }
